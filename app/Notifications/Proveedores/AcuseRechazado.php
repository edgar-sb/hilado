...
class AcuseRechazado extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        $mail = (new MailMessage)->markdown('vendor.notifications.email', [
            'compra' => $this->compra,
            'user' => $this->user,
            'estatus' => 'acuse-rechazado',
            'acuse_rechazo_estatus_log' => $this->estatus_log,
            'actionLines' => [
                'Por favor, ingresa en la plataforma y carga de nuevos los documentos.',
            ]
        ]);
        $mail->subject('Proveedor: Acuse rechazado OC: #'.$this->compra->no_compra);
        $mail->greeting('Hola '.$this->user->nombre.',');
        $mail->introLines = [
            'El acuse y carta porte que ingresaste a la orden de compra: #'.$this->compra->no_pedido.' han sido rechazados.',
            'Motivo de rechazo: '.$this->compra->rechazo_comentarios,
        ];
