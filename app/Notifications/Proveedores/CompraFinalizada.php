...
class CompraFinalizada extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        $mail->subject('Proveedor: Complemento de pago aprobado OC: #'.$this->compra->no_compra);
        $mail->greeting('Hola '.$this->user->nombre.',');
        $mail->introLines = [
            'EL complemento de pago que ingresaste ha sido aprobado.',
            'La orden de compra: #'.$this->compra->no_pedido.' se ha finalizado correctamente.',
        ];
